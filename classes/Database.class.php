...
#-#############################################
# desc: returns num of rows (not one row)
# param: (MySQL query) the query to run on server
# returns: assoc array of ALL fetched results
function fetch_num_rows($query_id=-1) {
	// retrieve row
	if ($query_id!=-1) {
		$this->query_id=$query_id;
	}
	if (isset($this->query_id)) {
			$count = @mysql_num_rows($this->query_id);
	}else{
		$this->oops("Invalid query_id: <b>$this->query_id</b>. Records could not be fetched.");
	}
...
#-#############################################
# desc: returns field name 
# param: (MySQL query) the query to run on server
# returns: assoc array of ALL fetched results
function fetch_field_name($query_id=-1,$val) {
	// retrieve row
	if ($query_id!=-1) {
		$this->query_id=$query_id;
	}
	if (isset($this->query_id)) {
			$fname = @mysql_field_name($this->query_id,$val);
	}else{
		$this->oops("Invalid query_id: <b>$this->query_id</b>. Records could not be fetched.");
	}
	$this->free_result($query_id);
